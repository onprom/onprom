...
class SimpleEBDAReasoner {
    ...
    SimpleEBDAReasoner(OBDAModel ebdaMapping, XFactory factory) {
        try {
            ...
            File file = generateMappingFile(ebdaMapping);

            OntopSQLOWLAPIConfiguration config = OntopSQLOWLAPIConfiguration.defaultBuilder()
                    .ontology(OWLManager.createOWLOntologyManager().loadOntologyFromOntologyDocument(SimpleEBDAReasoner.class.getResource(XESConstants.eventOntoPath).openStream()))
                    .nativeOntopMappingFile(file)
                    .jdbcUrl(dataSource.getParameter(RDBMSourceParameterConstants.DATABASE_URL))
                    .jdbcUser(dataSource.getParameter(RDBMSourceParameterConstants.DATABASE_USERNAME))
                    .jdbcPassword(dataSource.getParameter(RDBMSourceParameterConstants.DATABASE_PASSWORD))
                    .jdbcDriver(dataSource.getParameter(RDBMSourceParameterConstants.DATABASE_DRIVER))
                    .build();
    ...
    private File generateMappingFile(OBDAModel ebdaMapping) throws IOException {
        File file = File.createTempFile("obda-temp", System.currentTimeMillis() + ".obda");
        ModelIOManager manager = new ModelIOManager(ebdaMapping);
        manager.save(file);
        ...
        for (String line : lines) {
            if (line.trim().startsWith("[SourceDeclaration]")) {
                add = false;
            }
            if (line.trim().startsWith("[MappingDeclaration]")) {
                add = true;
            }
            if (add) fileWriter.write(line + "\n");
        }
        fileWriter.close();
