...
public class EventOntoVariantVToXESLogConverter {
	...
	private HashMap<String, XAttribute> getXAttributes(OWLOntology onto){
		...
		Set<OWLClassAssertionAxiom> ocaas = onto.getClassAssertionAxioms(new OWLClassImpl(IRI.create("http://onprom.inf.unibz.it/A")));
		...
		for(OWLClassAssertionAxiom ocaa : ocaas){
			...
			for(OWLDataPropertyAssertionAxiom dpaa : dpaas){
				...
				switch(dataProp.toString()){
					case "<http://onprom.inf.unibz.it/ak>": key = dataPropValue;break;
					case "<http://onprom.inf.unibz.it/at>": type = dataPropValue; break;
					case "<http://onprom.inf.unibz.it/av>": value = dataPropValue; break;
				}
			}
...
//			System.out.println("put into HashMap<String, XAttribute>: "+ i.toString() + "; "+ xatt);
	...
	private HashMap<String, XEvent> getXEvents(OWLOntology onto, HashMap<String, XAttribute> xatts){
		...
		// get all instances of the class "http://onprom.inf.unibz.it/Attribute"
		Set<OWLClassAssertionAxiom> ocaas = onto.getClassAssertionAxioms(new OWLClassImpl(IRI.create("http://onprom.inf.unibz.it/E")));
					...
					if(objProp != null){
						...
						if(objPropValue != null && !objPropValue.equals("")){
							switch(objProp.toString()){
								...
								case "<http://onprom.inf.unibz.it/e-a>": 
...
//			System.out.println("put into HashMap<String, XEvent>: "+ i.toString() + "; "+ xevt);
	...
	private HashMap<String, XTrace> getXTraces(OWLOntology onto, HashMap<String, XEvent> xevts, HashMap<String, XAttribute> xatts){
		...
		Set<OWLClassAssertionAxiom> ocaas = 
			onto.getClassAssertionAxioms(new OWLClassImpl(IRI.create("http://onprom.inf.unibz.it/T")));
				...
				if(odps.size() == 1){
					...
					if(objProp != null){
						...
						switch(objProp.toString()){
							...
							case "<http://onprom.inf.unibz.it/t-a>": 
							...
							case "<http://onprom.inf.unibz.it/t-e>": 
