 ...
 */
public class DynamicAnnotationforOCELEditor extends AnnotationEditor {
    private static final Logger logger = LoggerFactory.getLogger(DynamicAnnotationforOCELEditor.class.getName());
    private static final Map<String, UMLClass> annotations = new HashMap<>();

    public DynamicAnnotationforOCELEditor(OWLOntology upperOntology, OWLOntology domainOntology, AnnotationEditorListener _listener) {
    ...
    public static void main(String[] a) {
        new DynamicAnnotationforOCELEditor(null, null, null).display();
    }
    ...
    private void loadAnnotationTypes(OWLOntology upperOntology) {
        ...
        if (upperOntology != null) {
            ...
            OWLUtility.loadOntologyFromStream(DynamicAnnotationforOCELEditor.class.getResourceAsStream("/ocel-eo.owl"))
            ...
            setTitle("Default OCEL Annotation Editor");
        }
        initUI();
    }
    ...
    protected JToolBar createToolbar() {
        ...
        toolBar.add(UIUtility.createToolbarButton(loadOcelOntology()), 4);
    ...
    private ToolbarAction loadOcelOntology() {
        return new ToolbarAction(new AbstractActionType() {
            @Override
            public String getTooltip() {
                return "Load Default OCEL Ontology";
            }
    ...
    protected Collection<AnnotationProperties> getAnnotationProperties() {
        return annotations.values().stream().map(DynamicAnnotationforOCELEditor::getAnnotationProperties)
